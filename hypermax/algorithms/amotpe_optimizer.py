...
import logging
from hypermax.custom_sampler import CustomMOTPEMultiObjectiveSampler

# Suppress all messages from Optuna
optuna.logging.set_verbosity(optuna.logging.CRITICAL)
warnings.filterwarnings("ignore", category=DeprecationWarning, module="optuna")
# Suppress specific warnings by filtering them


class AMOTPEOptimizer(OptimizationAlgorithmBase):
    atpeParameters = [
        'gamma',
        'nEICandidates',
    ...
    def __init__(self):
        ...
        # self.study = optuna.multi_objective.create_study(
        #         directions=["minimize"],  # Use 'minimize' for both objectives
    ...
    def recommendNextParameters(self, hyperparameterSpace, results, obj_f, lockedValues=None, paramHistory=[]):
        rstate = numpy.random.RandomState(seed=int(random.randint(1, 2 ** 32 - 1)))
        warnings.filterwarnings("ignore", category=DeprecationWarning)
        warnings.filterwarnings("ignore", message=".*MultiObjectiveTrial.*deprecated.*")
        warnings.filterwarnings("ignore", message=".*MultiObjectiveStudy.*deprecated.*")

        def suggest_parameters(trial, hyperparameterSpace):
            params = {}
    
            for param in Hyperparameter(hyperparameterSpace).getFlatParameters():
                param_name = param.hyperoptVariableName
   
                config = param.config
      
                # Check parameter type and apply corresponding trial.suggest_* method
                if config['type'] == 'number' and config['mode'] == 'uniform':
                    # If the type is number and the mode is uniform, we use suggest_float for continuous values
                    params[param_name] = trial.suggest_float(param_name, config['min'], config['max'])
                elif config['type'] == 'int' and config['mode'] == 'uniform':
                    # If integer range, use suggest_int
                    params[param_name] = trial.suggest_int(param_name, config['min'], config['max'])
                elif config['type'] == 'categorical':
                    # If categorical, use suggest_categorical
                    params[param_name] = trial.suggest_categorical(param_name, config['choices'])
                # You can add more checks if there are other types or modes in your config
        ...
        def sample(trial):
            nonlocal params
            params = suggest_parameters(trial, hyperparameterSpace)
            evalParams = [params[f'root.{boundIndex}'] for boundIndex in range(len(params))]

            val = obj_f.evaluate(numpy.array(evalParams))
        ...
        def computePrimarySecondary():
            ...
            for parameter in numberParameters:
                if len(set(result[parameter.hyperoptVariableName] for result in results if result[parameter.hyperoptVariableName] is not None)) < 2:
                    correlations[parameter.hyperoptVariableName] = 0
                else:
                    values = []
                    valueLosses = []
                    for result in results:
                        if result[parameter.hyperoptVariableName] is not None and result['loss'] is not None:
                            values.append(result[parameter.hyperoptVariableName])
                    ...
                    correlations[parameter.hyperoptVariableName] = correlation
            ...
            if atpeParams['secondaryCutoff'] < 0:
                ...
                sortedParameters = sorted(numberParameters, key=lambda parameter: correlations[parameter.hyperoptVariableName])
            else:
            ...
            for parameter in sortedParameters:
                ...
                cumulative -= correlations[parameter.hyperoptVariableName]
            ...
            for secondary in secondaryParameters:
                if atpeParams['secondaryProbabilityMode'] == 'fixed':
                    if random.uniform(0, 1) < atpeParams['secondaryFixedProbability']:
                        self.lastLockedParameters.append(secondary.hyperoptVariableName)
                        if atpeParams['secondaryLockingMode'] == 'top':
                            lockResult = random.choice(topResults)
                            if lockResult[secondary.hyperoptVariableName] is not None and lockResult[secondary.hyperoptVariableName] != "":
                                lockedValues[secondary.hyperoptVariableName] = lockResult[secondary.hyperoptVariableName]
                        elif atpeParams['secondaryLockingMode'] == 'random':
                            lockedValues[secondary.hyperoptVariableName] = self.chooseRandomValueForParameter(secondary)
                    ...
                    probability = max(0, min(1, abs(correlations[secondary.hyperoptVariableName]) * multiplier))
        ...
        def convertResultsToOptunaTrials(hyperparameterSpace, results):
            trials = []
            
            for resultIndex, result in enumerate(results):
                # Create a dictionary of params and corresponding values from result
                params = {}
                distributions = {}
                for param in Hyperparameter(hyperparameterSpace).getFlatParameters():
                    # print(result)
                    value = result[param.hyperoptVariableName]
                    if value is not "" and value is not None:
                        if 'enum' in param.config:
                            # In case of enum, map back to index
                            value = param.config['enum'].index(value)
                        params[param.hyperoptVariableName] = value
                    # Add the distribution based on parameter type and mode
                    if param.config['type'] == 'number' and param.config['mode'] == 'uniform':
                        distributions[param.hyperoptVariableName] = optuna.distributions.UniformDistribution(param.config['min'], param.config['max'])
                    elif param.config['type'] == 'int' and param.config['mode'] == 'uniform':
                        distributions[param.hyperoptVariableName] = optuna.distributions.IntUniformDistribution(param.config['min'], param.config['max'])
                    elif param.config['type'] == 'categorical':
                        distributions[param.hyperoptVariableName] = optuna.distributions.CategoricalDistribution(param.config['enum'])

                # Construct an Optuna trial with the params and result values
                trial = optuna.trial.create_trial(
                    params=params,                          # Parameter values
                    values=[result['loss']],
                    distributions=distributions,                 # Objective value(s)
                    state=optuna.trial.TrialState.COMPLETE  # Assuming these are completed trials
                )
                trials.append(trial)

            return trials

        # print(atpeParams['gamma'])

        def gamma_function(n):
            return max(1, int(numpy.ceil(atpeParams['gamma'] * n)))

        # print(self.used)
        # new_sampler = optuna.multi_objective.samplers.MOTPEMultiObjectiveSampler(
        #         n_startup_trials= 0 if self.used else initializationRounds,
        #         n_ehvi_candidates=int(atpeParams['nEICandidates']),
        #         gamma=gamma_function
        #     )

        
        # print(self.study._storage.__dict__)

        # print(self.study._study.__dict__)
        if not self.used:
            self.motpe_params = {'gamma': atpeParams['gamma'], 'nEICandidates': atpeParams['nEICandidates']}
            print("init rounds1")
            print(initializationRounds)
            new_sampler = CustomMOTPEMultiObjectiveSampler(self.motpe_params, n_startup_trials=100)

            self.study = optuna.multi_objective.create_study(
            directions=["minimize", "minimize"],  # Keep the same directions
            sampler=new_sampler
        )
        else:
            self.motpe_params['gamma'] = atpeParams['gamma']
            self.motpe_params['nEICandidates'] = atpeParams['nEICandidates'] + random.randint(-10, 10)
        # else:
            # new_study = optuna.multi_objective.create_study(
            #     directions=["minimize", "minimize"],  # Keep the same directions
            #     sampler=new_sampler,
            #     study_name=self.study._study.study_name,  # Use the same study name
            #     storage=self.study._storage,  # Reuse the same storage (trials will persist)
            #     load_if_exists=True 
            # )
        self.used = True
        ...
        self.study.optimize(sample, n_trials=1, show_progress_bar=True)
    ...
    def computePartialResultStatistics(self, hyperparameterSpace, results):
        ...
        def getValue(result, parameter):
            return result[parameter.hyperoptVariableName]
